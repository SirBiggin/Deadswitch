...
class WebServer {
  ...
  // GET /api/messages — list all with recipients embedded
  static Future<Response> _getMessages(Request req) async {
    final db   = await DB.instance;
    final msgs = await db.query('messages', orderBy: 'name');
  ...
  // POST /api/messages — create with recipients
  ...
  // PUT /api/messages/:id — replace recipients
  static Future<Response> _updateMessage(Request req, String id) async {
    ...
    for (final r in (b['recipients'] as List? ?? [])) {
      await db.insert('message_recipients', {
        'message_id': mid, 'name': r['name'] ?? '', 'phone': r['phone'] ?? '',
      });
    }
    return _ok({'ok': true});
  }
