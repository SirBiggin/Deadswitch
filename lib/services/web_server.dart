...
class WebServer {
  ...
  static Future<void> start() async {
    ...
    router.post('/api/test-sms', _testSend);
  ...
  static Future<Response> _getGroups(Request req) async {
    ...
    for (final g in groups) {
      ...
      result.add({
        'id': g['id'], 'name': g['name'], 'message': g['message'],
        'recipients': recs.map((r) => {'id': r['id'], 'name': r['name'], 'phone': r['phone']}).toList(),
      });
    }
    return _ok(result);
  }
  ...
  static Future<Response> _createGroup(Request req) async {
    ...
    for (final r in (b['recipients'] as List? ?? [])) {
      await db.insert('group_recipients', {
        'group_id': gid, 'name': r['name'] ?? '', 'phone': r['phone'] ?? '',
      });
    }
